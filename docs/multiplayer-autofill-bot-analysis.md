...
- 화면 알림이 너무 자주 뜨지 않도록 `RemainingTime`은 30, 10, 5초에만 전송한다.
- 새 유저가 룸에 들어오거나 자동 출발이 다시 켜지면 카운트다운을 처음부터 다시 시작한다.
- 호스트가 `share_room`으로 모집을 다시 걸면 Starpoint HTTP 서버가 realtime 내부 control endpoint에 신호를 보내 카운트다운을 리셋한다.
