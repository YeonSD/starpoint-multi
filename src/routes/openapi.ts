...
const routes = async (fastify: FastifyInstance) => {
    ...
    fastify.post("/v3/promotion/getStartingPopups", (request: FastifyRequest, reply: FastifyReply) => {
        const body = request.body as PromotionBody
    ...
    fastify.post("/v3/log/writeRoundLog", (_, reply: FastifyReply) => {
        reply.status(200).send({
            "logId": randomUUID()
        })
    })
    ...
    fastify.post("/v3/player/getLocal", async (request: FastifyRequest, reply: FastifyReply) => {
        const body = request.body as PlayerGetLocalBody

        const rawAccountId = (request.headers['playerid'] as string | undefined) ?? body.playerId
        const accountId = rawAccountId ? Number.parseInt(rawAccountId) : undefined
        if (accountId === undefined || isNaN(accountId)) return reply.status(400).send({
            "error": "Bad Request",
